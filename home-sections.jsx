...
function WHero({ lang }) {
  ...
  return (
    <section style={{ position: 'relative', background: 'var(--color-bottle-black)', overflow: 'hidden', paddingTop: 72 }}>
      {/* W watermark */}
      <div style={{ position: 'absolute', fontFamily: 'var(--font-display)', fontSize: '52vw', fontWeight: 500, color: '#fff', opacity: 0.035, lineHeight: 0.8, right: '-6vw', bottom: '-14vw', userSelect: 'none', pointerEvents: 'none' }}>W</div>
      {/* autumn-vineyard video band, right side */}
      <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: '46%', opacity: 0.62 }}>
        <WVideo src={window.WMedia.heroVideo} poster={window.WMedia.heroPoster} label="VINEYARD · GOLDEN HOUR" tone="dark" position="center" />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, var(--color-bottle-black) 6%, transparent 72%)' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(0deg, var(--color-bottle-black) 2%, transparent 28%)' }} />
      </div>

      <div style={{ position: 'relative', zIndex: 1, maxWidth: MAXW, margin: '0 auto', padding: `clamp(70px,11vh,140px) ${PAD} clamp(56px,8vh,96px)`, opacity: "1" }}>
...
Object.assign(window, { Reveal, Over, WHero, WIntent, WTrust, WShop, WHotel, WWedding, WBiz, btn, sectionH2, sectionBody });